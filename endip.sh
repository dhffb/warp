...
endipresult() {
	...
	lowest_endpoint=$(cat result.csv | awk -F, '$3!="timeout ms" {print} ' | sort -t, -nk2 -nk3 | uniq | head -n 2 | tail -n 1 | awk -F, '{print $1}')
	echo "延迟最低的第一个结果为: $lowest_endpoint"
	applyCustomEndpoint $lowest_endpoint
	rm -rf ip.txt
	exit
}
